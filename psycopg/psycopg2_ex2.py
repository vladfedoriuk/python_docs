...
def insert_into_table(cur, form, table_name):
    ...
    insert_query = '(' + ', '.join(values.keys()) + ') VALUES (' + ', '.join(formatted) + ');'
    query_dict = {str(key) + "__val__": val for key, val in values.items()}
...
def get_hospital(hospital_id):
    with cursor as cur:
        if request.method == 'GET':
            cur.execute('SELECT * FROM hospitals WHERE id=%(id)s', {'id': str(hospital_id)})
            data = cur.fetchone()
            return jsonify({
                'id': data[0],
                'name': data[1],
...
def get_doctor(doctor_id):
    with cursor as cur:
        if request.method == 'GET':
            cur.execute('SELECT * FROM doctors WHERE id= %(id)s', {'id': str(doctor_id)})
            data = cur.fetchone()
            return jsonify({
                'id': data[0],
                'name': data[1],
...
def get_spec_doctor_list(speciality, salary):
    with cursor as cur:
        if request.method == 'GET':
            cur.execute('SELECT * FROM doctors WHERE salary >= %(salary)s AND speciality = %(speciality)s;',
                        {
                            'salary': salary,
...
@app.route('/doctors_in_hospital/<int:hospital_id>/', methods=['GET', ])
def doctors_in_hospital(hospital_id):
    with cursor as cur:
        if request.method == 'GET':
            cur.execute('SELECT * FROM doctors WHERE hospital_id = %(hospital_id)s;', {'hospital_id': hospital_id})
            doctors = cur.fetchall()
            return make_doctors_json(doctors)
