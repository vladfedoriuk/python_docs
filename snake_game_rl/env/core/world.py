...
from snake_game_rl.settings.constants import DIRECTIONS, SNAKE_SIZE, DEAD_REWARD, \
...
class World(object):
    def __init__(self, size, custom, start_position, start_direction_index, food_position):
        ...
        self.world[0, 0:] = self.WALL
        self.world[0:, 0] = self.WALL
        self.world[size[0] - 1, 0:] = self.WALL
    ...
    def init_food(self):
        ...
        if not self.custom:
            ...
            chosen_position = available_food_positions.pop()
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if new_snake_head[0] < 0 or new_snake_head[0] >= self.size[0] or new_snake_head[1] < 0 \
                    or new_snake_head[1] >= self.size[1]:
                self.snake.alive = False
            # Check if snake eats itself
            elif self.snake.blocks.count(new_snake_head) > 1:
                self.snake.alive = False
