...
def insert_many(db_name, values):
    with connect_to(db_name) as connection:
        with closing(connection.cursor()) as cursor:
            cursor.executemany("INSERT INTO person VALUES (?, ?, ?, ?, ?);", iter(values))
...
def update(db_name, id):
    with connect_to(db_name) as connection:
        with closing(connection.cursor()) as cursor:
            cursor.execute("UPDATE person SET salary = 1000, email = 'j@gmail.com' WHERE id = ?", (id,))


def update_many(db_name, records):
    with connect_to(db_name) as connection:
        with closing(connection.cursor()) as cursor:
            cursor.executemany("UPDATE person SET salary=? WHERE id = ? ", iter(records))
    ...
    with connect_to(db_name) as connection:
        with closing(connection.cursor()) as cursor:
            ...
            cursor.execute("""CREATE TABLE files(
                       ...
                       """)
            cursor.execute('INSERT INTO files VALUES(?, ?)', (filename, blob_data,))
...
def backup_example():
    ...
    with Connection(database='backup.db') as cursor:
        ...
        connection.backup(b_connection,
                          progress=lambda status, remaining, total: print(f'Copied {total - remaining} of {total}'))
...
if __name__ == '__main__':
    ...
    # select('test.db')
    records_to_insert = [(2, 'Jos', 'jos@gmail.com', '2019-01-14', 9500),
