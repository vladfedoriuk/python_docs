...
# writes, updates, and deetions operate only on the first mapping
...
words = "A Counter is a dict subclass for counting hashable objects. It is an unordered collection where elements are stored as dictionary keys and their counts are stored as dictionary values. " \
...
collections.Counter(dict([(1, 3), ('a', 3), ('b', 4)]))  # convert from a list of (elem, cnt) pairs
...
'is left unchanged. Deleting an entry and reinserting it will move it to the end.'
